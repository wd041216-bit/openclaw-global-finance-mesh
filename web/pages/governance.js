...
const shell = await initShell({
  pageId: "governance",
  sectionLabel: "治理中心",
  title: "只看审计链、导出与操作时间线",
  intro: "治理页只保留完整性、导出和 Operator Activity。备份与恢复演练已经独立搬到恢复中心。",
  heroActions: `
...
function renderFrame() {
    ...
    <section class="page-section">
      <div class="section-head compact">
        <div>
          ...
          <h3>治理动作时间线</h3>
          <p class="section-copy">仅 admin 可查看完整操作时间线。</p>
        </div>
  ...
  shell.pageContent.querySelector("#export-form")?.addEventListener("submit", onCreateExport);
...
function renderIntegrity() {
  ...
  const globalData = shell.getGlobal();
  shell.pageContent.querySelector("#verify-integrity").style.display = canManageAdmin(globalData) ? "" : "none";
  ...
  container.innerHTML = `
    ${summaryCard({
      ...
      note: state.integrity.lastVerifiedAt
        ...
        : "还没有执行过完整校验。",
      pillHtml: pill(state.integrity.isStale ? "warn" : state.integrity.status === "mismatch" ? "bad" : "good", state.integrity.status),
      meta: [
    ...
    ${detailRows([
      { label: "Verified Through", value: state.integrity.verifiedThroughSequence },
      { label: "Mismatch Count", value: state.integrity.mismatchCount },
      { label: "最近导出", value: state.integrity.lastExport?.backupId || state.integrity.lastExport?.id || state.integrity.lastExport?.createdAt },
    ])}
...
function renderExports() {
  ...
  const globalData = shell.getGlobal();
  shell.pageContent.querySelector("#export-form").closest(".page-section").style.display = canViewGovernance(globalData) ? "" : "none";
  if (!list || !detail) {
    return;
  }
  if (!canViewGovernance(globalData)) {
  ...
  if (!canManageAdmin(globalData)) {
    shell.pageContent.querySelector("#export-form").style.display = "none";
  }
